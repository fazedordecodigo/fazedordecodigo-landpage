...
const STATUS_TEXT = {
  ...
  failure:
    "Não foi possível enviar. Tente de novo ou fale com a gente no WhatsApp da comunidade.",
  sending: "Enviando…",
} as const;
...
function setStatus(
  ...
  const status = form.querySelector<HTMLElement>(FORM_SELECTORS.status);
  if (!status) return;
...
function syncCaptchaTheme(form: HTMLFormElement): void {
  const captcha = form.querySelector<HTMLElement>(FORM_SELECTORS.captcha);
  if (!captcha) return;
...
function resetCaptcha(): void {
  (window as Window & { hcaptcha?: HCaptchaApi }).hcaptcha?.reset();
}

async function submitForm(form: HTMLFormElement): Promise<void> {
  const button = form.querySelector<HTMLButtonElement>(FORM_SELECTORS.submitButton);
...
function onSubmit(event: SubmitEvent, form: HTMLFormElement): void {
  event.preventDefault();
  if (!form.reportValidity()) return;

  const captchaResponse = form.querySelector<HTMLTextAreaElement>(
    FORM_SELECTORS.captchaResponse,
  )?.value;
  if (!captchaResponse) {
  ...
  const accessKey = form.querySelector<HTMLInputElement>(FORM_SELECTORS.accessKey)
    ?.value;
...
function initContactForm(): void {
  const form = document.querySelector<HTMLFormElement>(FORM_SELECTORS.form);
  if (!form) return;
